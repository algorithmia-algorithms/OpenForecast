import re
import torch
from src.modules import data_proc, graph, misc, net_misc
...
def apply(input):
    ...
    if input['mode'] == "forecast":
        ...
        if guard.data:
            ...
            guard.data = data_proc.process_frames_incremental(guard.data, state, outlier_removal_multiplier, state["output_beam_width"])
        ...
        for i in range(forecasts.shape[0]):
            # forecast = revert_normalization(forecast, state)
            forecasts[i] = forecasts[i]
        envelope = graph.create_envelope(forecasts, guard.forecast_size)
        if guard.image_save_path:
            history = state['history'].cpu().data.numpy()
            # history = revert_normalization(history,  state)
            graph_path = graph.create_graph(envelope, history, guard.forecast_size, guard.io_noise)
        ...
        formatted_envelope = graph.ready_envelope(envelope, state['io_width'])
    ...
    if guard.mode == "train":
        ...
        if guard.checkpoint_input_path:
            local_file = data_proc.get_file(guard.checkpoint_input_path)
            network, state = net_misc.load_checkpoint(local_file)
            data = data_proc.process_frames_incremental(guard.data, state, gradient_multiplier, guard.future_beam_width)
        ...
        error, network = net_misc.train_autogenerative_model(data_frame=data, network=network,
                                                             ...
                                                             learning_rate=lr_rate, drop_percentage=guard.input_dropout)
        # _ = net_misc.evaluate_performance(data=data, network=network, checkpoint_state=state, length=150)
...
def test_train():
    ...
    input['checkpoint_output_path'] = "data://zeryx/forecasting_testing/csdisco_model.t7"
    ...
    input['io_noise'] = 0.03
    input['lookback_beam_width'] = 25
    input['future_beam_width'] = 10
...
def test_forecast():
    ...
    input['checkpoint_input_path'] = None
    ...
    # input['checkpoint_input_path'] = "data://zeryx/forecasting_testing/sinewave_incremented_model.t7"
    input['image_save_path'] = "data://zeryx/forecasting_testing/csdisco_forecast.png"
